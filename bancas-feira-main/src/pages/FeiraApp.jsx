...
export default function FeiraApp() {
  ...
  // Novos estados para totais de recebimentos de todos os usuários
  const [totalPixAllUsers, setTotalPixAllUsers] = useState(0);
  const [totalDinheiroAllUsers, setTotalDinheiroAllUsers] = useState(0);
  const [totalCartaoAllUsers, setTotalCartaoAllUsers] = useState(0);
  const [totalGeralAllUsers, setTotalGeralAllUsers] = useState(0);

  // Função para buscar e somar recebimentos de todos os usuários
  const fetchTotaisRecebimentosTodosUsuarios = useCallback(async () => {
    try {
      ...
      for (const usuarioDoc of usuariosSnapshot.docs) {
        const recebimentosRef = collection(db, "usuarios", usuarioDoc.id, "recebimentos");
      ...
      const totalSum = pixSum + dinheiroSum + cartaoSum;
      setTotalPixAllUsers(pixSum);
      setTotalDinheiroAllUsers(dinheiroSum);
      setTotalCartaoAllUsers(cartaoSum);
      setTotalGeralAllUsers(totalSum);

      // Atualizar totais no Firestore
      const totaisRef = doc(db, "totaisRecebimentos", "totais");
      await updateDoc(totaisRef, {
        pix: pixSum,
        dinheiro: dinheiroSum,
        cartao: cartaoSum,
        totalGeral: totalSum,
      }).catch(async (error) => {
        if (error.code === "not-found") {
          // Se o documento não existir, criar
          await setDoc(totaisRef, {
            pix: pixSum,
            dinheiro: dinheiroSum,
            cartao: cartaoSum,
            totalGeral: totalSum,
          ...
          console.error("Erro ao atualizar totais no Firestore:", error);
        }
      });

      setLoadingUsuarios(false);
    } catch (err) {
      alert("Erro ao buscar totais de recebimentos: " + err.message);
      setLoadingUsuarios(false);
    }
  }, [db, setLoadingUsuarios, setTotalPixAllUsers, setTotalDinheiroAllUsers, setTotalCartaoAllUsers, setTotalGeralAllUsers]);

  // Função para buscar totais do Firestore na inicialização
  const fetchTotaisDoBanco = useCallback(async () => {
    try {
      const totaisRef = doc(db, "totaisRecebimentos", "totais");
      const totaisSnap = await getDoc(totaisRef);
      if (totaisSnap.exists()) {
        const data = totaisSnap.data();
        setTotalPixAllUsers(data.pix || 0);
        setTotalDinheiroAllUsers(data.dinheiro || 0);
        setTotalCartaoAllUsers(data.cartao || 0);
        setTotalGeralAllUsers(data.totalGeral || 0);
      }
    } catch (error) {
  ...
  useEffect(() => {
    console.log("Usuário atual no useEffect de totais:", user ? user.email : "Nenhum usuário");
    if (user && user.email && user.email.toLowerCase() === "felipe@ipu.com") {
      console.log("Chamando fetchTotaisRecebimentosTodosUsuarios");
      fetchTotaisRecebimentosTodosUsuarios();
    }
  }, [user]);
  ...
  const saveRecebimento = async (userId, banca, paymentMethod, comprovanteHtml) => {
    try {
      ...
      // Verificar se já existe um recebimento para esta banca e método no mesmo dia para evitar duplicidade
      const snapshot = await getDocs(recebimentosRef);
      const hoje = now.toISOString().slice(0, 10); // yyyy-mm-dd
      const recebimentoExistente = snapshot.docs.find(doc => {
        const dataDoc = doc.data();
        const dataRecebimento = dataDoc.data ? dataDoc.data.slice(0, 10) : null;
        return dataRecebimento === hoje && dataDoc.bancaNumero === banca.numero && dataDoc.paymentMethod === paymentMethod;
      });

      if (recebimentoExistente) {
        console.log("Recebimento já registrado para esta banca, método e dia. Ignorando duplicidade.");
        return;
      }
                    ...
                    onClick={() => {
                      const printContent = `
                        <html>
                          ...
                          <body>
                            <h1>Totais Recebidos</h1>
                            <p>Email do usuário: ${selectedUser ? selectedUser.email : "Não informado"}</p>
